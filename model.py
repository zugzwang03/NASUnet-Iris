...
history = best_model.fit(
    train[0], train[1], batch_size=32, epochs=30, validation_split=0.1
)
...
plt.plot(
    history.history["accuracy"], label="Training Accuracy", color="blue", linewidth=2
)
plt.plot(
...
predictions = (predictions > np.mean(predictions)).astype(np.uint8)
...
def save_predictions(predictions, output_folder):
    ...
    for i, pred in enumerate(predictions):
        ...
        pred_image_resized_lanczos = cv2.resize(
            pred_image, (256, 256), interpolation=cv2.INTER_LANCZOS4
        )
        # Apply Bilateral filter
        pred_image_resized_bilateral = cv2.bilateralFilter(
            pred_image_resized_lanczos, d=9, sigmaColor=75, sigmaSpace=75
        )
